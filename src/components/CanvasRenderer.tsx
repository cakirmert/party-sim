  ...
  function renderCanvas() {
    ...
    for (let y = 0; y < engine.map.height; y++) {
      for (let x = 0; x < engine.map.width; x++) {
        ...
        if (t.tag && t.tag !== "BUILDABLE" && t.tag !== "BED") {
          ...
          if (labelNames[t.tag]) {
            ...
            labelAccumulator.set(key as TileTag, entry);
                ...
                // explicitly re-draw BEDs on top to override any tint
        if (t.tag === "BED") {
          ctx.fillStyle = "#e63946"; // or "red"
          ctx.fillRect(x * ppt, y * ppt, ppt, ppt);
          ctx.strokeStyle = "#b91c1c"; // optional darker outline
          ctx.lineWidth = 1;
